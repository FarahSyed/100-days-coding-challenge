...
function getNextBirthday(month: number, date: number) {
    ...
    const birthday = new Date(year, month - 1, date);   // month - 1 as it starts with 0 index
    
    // If birthday has already passed this year set next year
    if (birthday < currentDate) birthday.setFullYear(year + 1);
    
    return birthday;
}

const nextBirthday = getNextBirthday(4, 14);   // April, 14
...
const birthday2 = new Date(2024, 3, 14);    // 13   Parameters could equally be: (`April 14, 2024`)
...
const birthday3 = new Date(Date.UTC(2024, 3, 14));
// Alternatively, explicitly setting the time to 12 PM (noon) avoids timezone discrepancies
const birthday4 = new Date(2024, 3, 14, 12, 0, 0);    // OR (`April 14, 2024 12:00:00`)
...
const birthday5 = new Date(`2024-04-14`);