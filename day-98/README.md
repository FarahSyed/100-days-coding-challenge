...
In this chapter, you'll add authentication to your dashboard using NextAuth.js. Here's what we will cover:
...
Authentication is how a system checks if a user is who they claim to be. It often involves multiple methods such as passwords and verification codes (2-factor authentication).
...
Update `auth.ts` to add the Credentials provider:
...
Update the login form to handle authentication:
...
Update the `SideNav` component to handle sign out:
```tsx
...
### Try It Out
You should now be able to log in and out of your application using the following credentials:
